...
(async function testLogin() {
    ...
    try {
        ...
        const lanzamientosButton = await driver.wait(until.elementLocated(By.id('radix-:r0:-trigger-lanzamientos')), 5000);
        
        // Hacer clic en el botón si se encuentra
        if (lanzamientosButton) {
            await lanzamientosButton.click(); // Hacer clic en el botón
            await driver.sleep(2000);
            console.log("Test logrado: El botón 'LANZAMIENTOS' fue encontrado y clickeado.");
            console.log('Prueba exitosa: PBI 14 realizada');
        } else {
            console.log("El botón 'LANZAMIENTOS' no fue encontrado.");
        }
