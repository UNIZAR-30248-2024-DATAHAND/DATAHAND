...
(async function testPlayerProfile() {
    ...
    try {
        ...
        const buttonVer = await driver.wait(
            until.elementLocated(By.xpath("//div[contains(@class, 'flex justify-between items-center p-2 border-b border-gray-400 mb-2 bg-gray-300 rounded')]//p[contains(text(), 'Partido-100')]/following-sibling::div//button[contains(text(), 'Editar')]")),
            5000
        );

        console.log('El botón "Editar" está presente para "Partido-100".');

        // Hacer clic en el botón "Ver"
        await buttonVer.click();


        await driver.wait(until.urlIs('http://localhost:3000/register-match/Partido-100'), 5000);
