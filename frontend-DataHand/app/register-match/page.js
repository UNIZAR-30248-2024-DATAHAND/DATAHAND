...
export default function Home() {
  ...
  const [sistemaDefensivoVisitante, setSistemaDefensivoVisitante] =
    useState(null);
  ...
  const intercambiarPosiciones = (equipo, index, tipo) => {
    ...
    const jugadorActual =
      tipo === 'jugador'
        ? equipoSeleccionado.jugadores[index]
        : tipo === 'portero'
    ...
    if (
      esPortero(jugadorSeleccionado) &&
      (tipo === 'jugador' || tipo === 'banquillo')
    ) {
      alert('No se puede intercambiar un portero con un jugador.');
      return;
    }
    if (
      esPortero(jugadorActual) &&
      (seleccionado.tipo === 'jugador' || seleccionado.tipo === 'banquillo')
    ) {
    ...
    if (seleccionado.tipo === tipo) {
      if (tipo === 'jugador') {
        ...
        [nuevosJugadores[seleccionado.index], nuevosJugadores[index]] = [
          nuevosJugadores[index],
          nuevosJugadores[seleccionado.index],
        ];
        setEquipos({
          ...equipos,
        ...
        [nuevosPorteros[seleccionado.index], nuevosPorteros[index]] = [
          nuevosPorteros[index],
          nuevosPorteros[seleccionado.index],
        ];
        setEquipos({
          ...equipos,
        ...
        [nuevoBanquillo[seleccionado.index], nuevoBanquillo[index]] = [
          nuevoBanquillo[index],
          nuevoBanquillo[seleccionado.index],
        ];
        setEquipos({
          ...equipos,
      ...
      if (tipo === 'jugador') {
        const nuevosJugadores = [...equipoSeleccionado.jugadores];
        const nuevoBanquillo = [...equipoSeleccionado.banquillo];
        [nuevoBanquillo[seleccionado.index], nuevosJugadores[index]] = [
          nuevosJugadores[index],
          nuevoBanquillo[seleccionado.index],
        ];
        setEquipos({
          ...
          [equipo]: {
            ...
            banquillo: nuevoBanquillo,
          },
        });
      } else if (tipo === 'banquillo') {
        const nuevosJugadores = [...equipoSeleccionado.jugadores];
        const nuevoBanquillo = [...equipoSeleccionado.banquillo];
        [nuevosJugadores[seleccionado.index], nuevoBanquillo[index]] = [
          nuevoBanquillo[index],
          nuevosJugadores[seleccionado.index],
        ];
        setEquipos({
  ...
  const esPortero = (jugador) => {
    return (
      equipos.local.porteros.includes(jugador) ||
      equipos.visitante.porteros.includes(jugador)
    );
  };
  ...
  const iniciarCronometro = () => {
    if (!sistemaDefensivoLocal || !sistemaDefensivoVisitante) {
      alert(
        'Debes seleccionar un sistema defensivo antes de comenzar el partido.'
      );
      return;
    }
  ...
  return (
    <div className="relative h-screen flex flex-col items-center justify-start bg-orange-500 overflow-hidden p-4 overflow-y-auto    ">
      ...
      <div className="w-full h-32 bg-white rounded-lg flex items-center justify-between p-4 mb-8 shadow-md">
        ...
        <div className="flex items-center">
          <div className="flex flex-col mr-4">
            <span className="text-lg font-semibold text-black">
              Cronómetro: {formatearTiempo(tiempo)}
            </span>
          ...
          <div className="flex gap-2">
            <button
              className={`mt-4 px-4 py-2 rounded bg-blue-500 text-white ${
                !sistemaDefensivoLocal || !sistemaDefensivoVisitante
        ...
        <Link href="/">
          <button className="bg-red-600 text-white border-2 border-white p-4 rounded-full font-semibold hover:bg-white hover:text-purple-600 transition duration-300 ease-in-out text-center w-full">
            Salir
          </button>
        </Link>
      </div>
      ...
      <div className="flex justify-between w-full mb-12 flex-grow">
        ...
        <div className="flex-1 h-[calc(100vh-256px)] bg-white rounded-lg shadow-md mx-2 p-4 flex flex-col">
          ...
          <div className="mt-4">
            ...
            <div className="flex justify-between mb-2">
              <button className="bg-blue-500 text-white px-3 py-3 rounded-lg">
                Primero
              </button>
              <button className="bg-green-500 text-white px-3 py-3 rounded-lg">
                Segundo
              </button>
              <button className="bg-red-500 text-white px-3 py-3 rounded-lg">
                Tercero
              </button>
            </div>
          </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {equipos.local.banquillo.map((jugador, index) => (
                <button
                  key={index}
                  onClick={() =>
                    seleccionarJugador('local', index, 'banquillo')
                  }
                  className={`${
                    seleccionado?.equipo === 'local' &&
                    seleccionado?.index === index &&
                    seleccionado?.tipo === 'banquillo'
                      ? 'bg-red-500'
                      : 'bg-yellow-500'
                  } text-white px-3 py-3 rounded-lg`}
                >
              ...
              >
                ...
                {/* Segundo portero en el banquillo */}
              </button>
            </div>
          </div>
        ...
        <div className="flex-[1.5] h-[calc(100vh-256px)] bg-white rounded-lg shadow-md mx-2 p-4 flex flex-col">
          ...
          <div className="mt-4">
            ...
            <div className="flex justify-between mb-2">
              {['Gol', 'Parada', 'Palo/Fuera', 'Perdida de balon'].map(
                (opcionResultado) => (
                  <button
                    key={opcionResultado}
        ...
        <div className="flex-1 h-[calc(100vh-256px)] bg-white rounded-lg shadow-md mx-2 p-4 flex flex-col">
          ...
          <div className="mt-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                Portero {equipos.visitante.porteros[0]}{' '}
                {/* Primer portero visible */}
              </button>
            </div>
          </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {equipos.visitante.jugadores.map((jugador, index) => (
                <button
                  key={index}
                  onClick={() =>
                    seleccionarJugador('visitante', index, 'jugador')
                  }
                  className={`${
                    seleccionado?.equipo === 'visitante' &&
                    seleccionado?.index === index &&
            ...
            <div className="grid grid-cols-3 gap-2">
              {equipos.visitante.banquillo.map((jugador, index) => (
                <button
                  key={index}
                  onClick={() =>
                    seleccionarJugador('visitante', index, 'banquillo')
                  }
                  className={`${
                    seleccionado?.equipo === 'visitante' &&
                    seleccionado?.index === index &&
              ...
              >
                Portero {equipos.visitante.porteros[1]}{' '}
                {/* Segundo portero en el banquillo */}
              </button>
            </div>
          </div>
              ...
              {['6:0', '5:1', '3:2:1', '4:2', 'Otros'].map((opcion, index) => (
                <div key={index} className="flex items-center">
                  <input
                    ...
                    onChange={() =>
                      seleccionarSistemaDefensivoVisitante(opcion)
                    } // Actualiza el estado al seleccionar
                    checked={sistemaDefensivoVisitante === opcion} // Verificar cuál está seleccionado
                  />
                  <label
                    htmlFor={`radio-visitante-${index}`}
                    className="text-black text-lg"
                  >
                    {opcion}
                  </label>
              ...
              <div className="flex flex-col flex-1 justify-between">
                ...
                <div className="bg-gray-200 rounded-lg p-4 mb-4 flex-grow text-center">
                  ...
                  <p className="text-black">
                    Detalles sobre la posición del gol.
                  </p>
                </div>
                ...
                <div className="bg-gray-200 rounded-lg p-4 flex-grow text-center">
                  ...
                  <p className="text-black">
                    Detalles sobre la posición del lanzador.
                  </p>
                </div>
              </div>
